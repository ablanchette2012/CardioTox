...
make_stan_dat <- function(dat,chemnum, parmcol, parmname="", parmnamenorm="",
  ...
  cat("##############",fileprefix,"\n")
  stan_dat <- list(
    scale_factor = median(dat$y,na.rm=TRUE), # dat not dat.tmp ##need to change number of cell lines and datapoint? need to make big changes ? 
    Ni = length(unique(dat$cell.f)), # dat not dat.tmp  
    Nj = dim(dat.tmp)[1],
    x = dat.tmp$Chemical.Concentration,
    ys = dat.tmp$y/median(dat$y,na.rm=TRUE),  ##change to sim responses? rnorm with set mean and sd for now? 
...
parmnamenormvec<-c("Peak Amplitude dn","BPM up","BPM dn",
                  "Peak Amplitude CV up","Peak Spacing CV up",
                  "Peak Decay-Rise Ratio up")
...
for (k in 1:6) {
  ...
  for (chemnum in c(5,6,15,37,2,34,35,29,28,12)) {
