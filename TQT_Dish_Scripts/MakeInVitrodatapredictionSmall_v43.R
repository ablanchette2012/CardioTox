...
for (j in 1:nrow(chemmap)) {
  ...
  #### load parameters***
  y0<-as.data.frame(rbind((eval(parse(text=files[1])))[2:2001, 189:231], (eval(parse(text=files[2])))[2:2001, 189:231], (eval(parse(text=files[3])))[2:2001, 189:231], (eval(parse(text=files[4])))[2:2001, 189:231]))
  colnames(y0)<- (1:43)
  rownames(y0)<- c(1:8000)
  parmframe<- melt(as.matrix(y0))
  colnames(parmframe)<- c("Iter", "ID", "y0")
  x0<-as.data.frame(rbind((eval(parse(text=files[1])))[2:2001, 232:274], (eval(parse(text=files[2])))[2:2001, 232:274], (eval(parse(text=files[3])))[2:2001, 232:274], (eval(parse(text=files[4])))[2:2001, 232:274]))
  colnames(x0)<- paste("ID",1:43, sep="")
  rownames(x0)<- c(1:8000)
  parmframe$x0<- melt(as.matrix(x0))$value
  Emax<-as.data.frame(rbind((eval(parse(text=files[1])))[2:2001, 275:317], (eval(parse(text=files[2])))[2:2001, 275:317], (eval(parse(text=files[3])))[2:2001, 275:317], (eval(parse(text=files[4])))[2:2001, 275:317]))
  colnames(Emax)<- paste("ID",1:43, sep="")
  rownames(Emax)<- c(1:8000)
  parmframe$Emax<- melt(as.matrix(Emax))$value
  n<-as.data.frame(rbind((eval(parse(text=files[1])))[2:2001, 318:360], (eval(parse(text=files[2])))[2:2001, 318:360], (eval(parse(text=files[3])))[2:2001, 318:360], (eval(parse(text=files[4])))[2:2001, 318:360]))
  ...
  for (i in 1:nrow(concframe)){
    ...
    parmframeconc$concentration<-concframe$x[i]
    ...
    Prediction_Frame<- rbind(Prediction_Frame,
                             cbind(predframe_median,
                                   predframe_25$Pred,
                                   predframe_975$Pred
                                   ))
...
save(data.list,chemmap,DR_Frame_All,
     DR_Frame_Controls,
     DR_Frame_Treated,
     file="InVitrodatandpredictionSmall_43indiv.Rdata")
