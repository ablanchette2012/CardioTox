...
parameters {
	...
	real m_Emax;	// max effect size
	real m_n;	// Hill exponent
	// Population standard deviation
	real<lower=0> sd_y0;
	real<lower=0> sd_x0;
	real<lower=0> sd_Emax;
	real<lower=0> sd_n;
...
transformed parameters {
	...
	real Emax[Ni];	// untransformed max effect size
	real n[Ni];		// untransformed Hill exponent
	for (i in 1:Ni) { // un-log transform
		y0[i] = exp(m_y0 + sd_y0 * z_y0[i]);
		x0[i] = exp(m_x0 + sd_x0 * z_x0[i]);
		Emax[i] = inv_logit(m_Emax + sd_Emax * z_Emax[i]); // Emax between 0 and 1
...
model {
	...
	for (j in 1:Nj) {
		yp[j] = y0[cell[j]]*(1 + (x[j] / x0[cell[j]])^n[cell[j]] / (1 + ((x[j] / x0[cell[j]])^n[cell[j]])/Emax[cell[j]]));
	}
	ys ~ student_t(5,yp,sigma_y);
}
generated quantities {
	vector[Ni] ec100;		// Concentration at which the relative response is 100%
...
//	real ec10_GSD_approx;		// Approximate GSD derived from m_0,m_1,sd_0, and sd_1
	real ec100_median;			// Median ec100
	...
	vector[Nquants] ec100_quants;	// Quantiles ec100
	vector[Nquants] ec100_quant_ratios;	// Ratios of median to quantiles ec100
	...
	for (i in 1:Ni) {
		if (Emax[i] > 1) 
			ec100[i] = x0[i]*(1 * Emax[i] / (Emax[i] - 1))^(1/n[i]);
		else
			ec100[i] = 1000;
	}
	for (i in 1:Ni) {
	...
	for (i in 1:Ni) {
		if (Emax[i] > 0.1)
			ec10[i] = x0[i]*(0.1 * Emax[i] / (Emax[i] - 0.1))^(1/n[i]);
		else
			ec10[i] = 1000;
	}
	for (i in 1:Ni) {
		if (Emax[i] > 0.05) 
			ec05[i] = x0[i]*(0.05 * Emax[i] / (Emax[i] - 0.05))^(1/n[i]);
		else
			ec05[i] = 1000;
	}
	for (i in 1:Ni) {
		if (Emax[i] > 0.025)
			ec025[i] = x0[i]*(0.025 * Emax[i] / (Emax[i] - 0.025))^(1/n[i]);
		else
			ec025[i] = 1000;
	}
	for (i in 1:Ni) {
		if (Emax[i] > 0.01) 
			ec01[i] = x0[i]*(0.01 * Emax[i] / (Emax[i] - 0.01))^(1/n[i]);
		else
			ec01[i] = 1000;
	}
//	ec10_GM = exp(mean(log(ec10)));
//	ec10_GSD = exp(sd(log(ec10)));
//	ec10_GSD_approx = exp(sqrt((sd_0 / m_1)^2 + (sd_1 / m_1)^2 * ((log(0.1/(1-0.1)) - m_0)/m_1)^2));
	{
	int ec100_indx[Ni];
	int ec50_indx[Ni];
	int ec10_indx[Ni];
	...
	int i;
	int itmp;
	ec100_indx= sort_indices_asc(ec100);
	...
	i = itmp-1;
	g = Ni*p + m - i;
	ec100_median = (1-g)*ec100[ec100_indx[i]]+g*ec100[ec100_indx[i+1]];
	...
	for (q in 1:Nquants) {
		...
		i = itmp-1;
		g = Ni*p + m - i;
		ec100_quants[q] = (1-g)*ec100[ec100_indx[i]]+g*ec100[ec100_indx[i+1]];
	...
	for (q in 1:Nquants) { 
		ec100_quant_ratios[q] = ec100_median/ec100_quants[q];
