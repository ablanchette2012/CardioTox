...
ChemKey<-read.table("chemicals.dat", header=FALSE, fill=TRUE)
...
make_stan_dat <- function(dat,chemnum, parmcol, parmname="", parmnamenorm="",
  ...
  ###
  fileprefix<-paste(chemnum, "_", "standat_v4_do_stan_fit_",parmnamenorm,sep="")
  cat("##############",fileprefix,"\n")
  stan_dat <- list(
...
do_stan_fit <- function(stan_dat, fileprefix="",direction = 1,
  ...
  if (direction == 1) {
    ...
    stanmodel <- 'C:\\Users\\ablanchette\\Documents\\CardioTox\\ExpandedScreen\\dose_response\\conc_resp_v4_up.stan' 
  } else {
    stanmodel <- 'C:\\Users\\ablanchette\\Documents\\CardioTox\\ExpandedScreen\\dose_response\\conc_resp_v4_dn.stan' 
  }
