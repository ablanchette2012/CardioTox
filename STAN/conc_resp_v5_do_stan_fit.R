...
# modelname<-"conc_resp_v5"
#includes post modeling CSV processing 
library(parallel)
library(rstan)
library(MASS)
library(Hmisc) 
...
##retrieve chemical list
ChemKey<-read.table("C:\\Users\\ablanchette\\Documents\\CardioTox\\ExpandedScreen\\dose_response\\CO_Test\\chemicals.dat", header=FALSE, fill=TRUE)

colnames(ChemKey)<- c("Chemical.Number", "Chemical.Name")
#change path accordingly
get.chem.map <- function(filepath="C:\\Users\\ablanchette\\Documents\\CardioTox\\ExpandedScreen\\dose_response\\") {  
...
make_stan_dat <- function(dat,chemnum, parmcol, parmname="", parmnamenorm="",
  ...
  ###
  fileprefix<-paste("C", chemnum,"_","standat_v5_do_stan_fit_",parmnamenorm,sep="")
  cat("##############",fileprefix,"\n")
  stan_dat <- list(
  ...
  #change wd to chem specific folder
  setwd(chempath)
  with(stan_dat, {
...
do_stan_fit <- function(stan_dat, fileprefix="",direction = 1,
  ...
  if (direction == 1) {
    ...
    stanmodel <- 'C:\\Users\\ablanchette\\Documents\\CardioTox\\ExpandedScreen\\dose_response\\CO_Test_2\\conc_resp_v5_up.stan' 
  } else {
    stanmodel <- 'C:\\Users\\ablanchette\\Documents\\CardioTox\\ExpandedScreen\\dose_response\\CO_Test_2\\conc_resp_v5_dn.stan' 
  }
  
  setwd(chempath)
  
  time.start<-proc.time()
  stan_fit <- stan(file=stanmodel,data=stan_dat,verbose=verbose,seed=seed,iter=iter, save_warmup=FALSE, cores = getOption("mc.cores", 4L),
...
get_stan_fit_csv <- function(stan_dat,fileprefix,direction) {
  stan_fit <- read_stan_csv(dir(pattern=paste(fileprefix,"_samples_","[[:digit:]]",sep=""), 
                                full.names = TRUE))
...
plot_stan_fit_pop <- function(stan_fit,fileprefix, dopdf=FALSE) { ##same
  ...
  print(stan_plot(stan_fit,par="ec100")+ ggtitle(fileprefix))
...
plot_stan_fit_iter <- function(stan_samples,stan_dat,direction,chemname,parmnamenorm, 
  ...
  for (i in 1:stan_dat$Ni) {
    ...
    xec100<-10^seq(max(log10(xmin),quantile(log10(stan_samples$ec100[,i]),prob=c(0.05))),
                  quantile(log10(stan_samples$ec100[,i]),prob=c(0.95)),
                  length.out=100)
    yec100<-stan_dat$scale_factor*rep(median(stan_samples$y0[,i])*(1+0.1*direction),100)
    points(xec100,yec100,pch=15)
    points(xec100,yec100,pch=15,col="grey",cex=0.5)
...
if (file.exists("Peak.Parms.All.NotchCO.Clean.dat") == TRUE){
  
  dat<-read.table("Peak.Parms.All.NotchCO.Clean.dat",header=TRUE,as.is=TRUE,sep="\t")

  } else {

  alldat<-read.table("Peak.Parms.All.NotchCO.dat",header=TRUE,as.is=TRUE,sep="\t")
  ...
  write.table(dat.clean,"Peak.Parms.All.NotchCO.Clean.dat",sep="\t",row.names=FALSE)
...
for (k in c(1,3,5)) {
  ...
  for (chemnum in c(4,27,30)) {
    
    # Create directory for each chemical and output files 
...
chemicals<- read.table("chemicals.dat", as.is = TRUE, header = FALSE)
...
for (i in c(4,27,30)) {
  ...
  for (f in 1:length(files)){
    ...
    dat.tmp<- read.csv(file.name, skip = 25, header = FALSE, as.is = TRUE)
